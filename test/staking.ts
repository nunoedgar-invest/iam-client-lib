import { StakingPoolFactory__factory, IRoleDefinition, RewardPool__factory, StakingPool__factory, VOLTA_CHAIN_ID } from "@energyweb/iam-contracts";
...
export const stakingTests = (): void => {
  ...
  describe.only("Tests on VOLTA", () => {
    it("Full staking workflow", async () => {
      ...
      const FAST_REWARD_POOL_ADDRESS = "0x0Bee80D73e141ed2a49D5F310938E97E573D97Bb";
      setChainConfig(VOLTA_CHAIN_ID, { stakingPoolFactoryAddress: STAKING_POOL_FACTORY_FAST_REWARD });
      const faucet = new Wallet("1aec3458500362c0a0f1772ab724a71b0f9d7da418a2d86d5954ab3f4b58ec4e").connect(provider);
      const patron = faucet;
      const amount = parseEther("1");
      await (await faucet.sendTransaction({ to: FAST_REWARD_POOL_ADDRESS, value: parseEther("10") })).wait();
      ...
      if (pool) {
        ...
        if (stake.status === StakeStatus.NONSTAKING) {
          console.log(">>> putting stake...");
          await pool.putStake(amount);
          stake = await pool.getStake();
        }
        if (stake.status === StakeStatus.STAKING) {
          let requestDelay = await pool.requestWithdrawDelay();
          console.log(">>> waiting to request withdraw: ", requestDelay.toNumber());
          await new Promise((resolve) => {
            setTimeout(resolve, 1000 * requestDelay.toNumber());
          });
          requestDelay = await pool.requestWithdrawDelay();
          if (requestDelay.eq(0)) {
        ...
        if (stake.status === StakeStatus.WITHDRAWING) {
          let withdrawDelay = await pool.withdrawalDelay();
          console.log(">>> waiting to withdraw: ", withdrawDelay.toNumber());
          await new Promise((resolve) => {
            setTimeout(resolve, 1000 * withdrawDelay.toNumber());
          });
          withdrawDelay = await pool.withdrawalDelay();
          if (withdrawDelay.eq(0)) {
